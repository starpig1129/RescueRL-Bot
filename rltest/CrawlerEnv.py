...
class CrawlerEnv(gym.Env):
    def __init__(self, show):
        ...
        self.control_conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        self.control_conn.settimeout(10.0)  # 設定接收超時
        print("已連接到控制伺服器:", self.control_addr)
        
        # 建立數據回傳伺服器
        self.info_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.info_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        ...
        self.info_conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        self.info_conn.settimeout(10.0)  # 設定接收超時
        print("已連接到數據伺服器:", self.info_addr)

        # 建立影像接收伺服器
        self.obs_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.obs_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        ...
        self.obs_conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        self.obs_conn.settimeout(10.0)  # 設定接收超時
        print("已連接到影像接收伺服器:", self.obs_addr)

        # 建立重置訊號接收伺服器
        self.reset_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.reset_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法
        self.reset_address = ('localhost', 7000)
        self.reset_socket.bind(self.reset_address)
        self.reset_socket.listen(5)
        self.reset_socket.settimeout(10.0)  # 設定接收超時
